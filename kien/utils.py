    ...
    def decorator(func):
        ...
        def wrapper(command, *args, **kwargs):
            if isinstance(command, str):
                ...
                return func(shlex.split(command), *args, **kwargs)
            else:
...
def join_generator_string(
    glue: str = os.linesep, formatter: Optional[Callable[[str], str]] = None
...
class FragileStreamHandler(logging.StreamHandler):
    ...
    This is relevant for interfaces, that may disconnect at any time (e.g. a USB gadget
    interface).
...
class CommandExecutionContext(AbstractContextManager):  # type: ignore
    """
...
def back_pressure(commander, strategy: Type[BackPressure] = RateLimitBackPressure):
    """
    Decorator that implements back-pressure for commands. As back pressure always means
    that the amount of data that is generated by the command MUST be reduced it should
    only be used if the command generates results that can be dropped arbitrarily or if
    the back pressure strategy can adequately decide which results are insignificant.

    :param commander: the commander used to register the command
    :param strategy: the back-pressure strategy that should be used to reduce the output
        flow
    """
